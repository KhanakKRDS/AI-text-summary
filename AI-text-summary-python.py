...
tokenizer = AutoTokenizer.from_pretrained ("../My-tune-model./fine-tune-BART") #BART model
model = AutoModelForSeq2SeqLM.from_pretrained("../My-tune-model./fine-tune-BART")
...
def index(): #on a post request, it retrieves user input, process the text to extract
    ...
    return render_template("./Website./AI-text-summary.html", og = "", summary = "")
...
def process(): #handles user input, process the text, and displays the summary
    ...
    return render_template("./Website./AI-text-summary.html", og = original_text, summary = summary)
